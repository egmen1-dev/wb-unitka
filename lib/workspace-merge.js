...
/**
 * Сохранение в облако: непустой incoming — авторитетный список (удаления сохраняются).
 * Пустой incoming при наличии профилей в облаке — участник без ключей, не затираем команду.
 */
export function reconcileWorkspaceProfiles(existing = [], incoming = []) {
  const existingList = Array.isArray(existing) ? existing : [];
  const incomingList = Array.isArray(incoming) ? incoming : null;
  if (!incomingList) return existingList;
  if (incomingList.length === 0 && existingList.length > 0) return existingList;

  const existingById = new Map();
  for (const profile of existingList) {
    if (profile?.id) existingById.set(profile.id, profile);
  }

  return incomingList
    .filter((profile) => profile?.id)
    .map((incomingProfile) => {
      const prev = existingById.get(incomingProfile.id);
      if (!prev) return incomingProfile;
      return {
        ...prev,
        ...incomingProfile,
        token: pickProfileToken(prev, incomingProfile),
        name: incomingProfile.name || prev.name,
      };
    });
}
...
export function mergeWorkspacePayload(existing = {}, incoming = {}) {
  ...
  next.profiles = reconcileWorkspaceProfiles(existing.profiles, incoming.profiles);
