...
import {
  ...
  resolveMatrixCargoTypeFromMeta,
...
import { suggestWarehousesForLocation, isFederalDistrictLabel } from './wb-region-sales.js';
import { buildWarehouseFoResolver, defaultWarehousesForFo, foZoneKey } from './wb-warehouse-fo.js';
...
function resolveTariffs(tariffCache, cargoType = WB_CARGO.MGT, rows = [], settings = {}) {
  ...
  const boxWarehouses =
    hydrated?.boxTariffs?.warehouses || tariffCache?.boxWarehouses || [];
  if (boxWarehouses.length) {
    for (const row of boxWarehouses) {
      const tariff = row?.warehouseName ? row : null;
      if (!tariff?.warehouseName || !warehouseAcceptsCargoType(tariff.warehouseName, cargoType)) {
        continue;
      }
      const key = normalizeWarehouseKey(tariff.warehouseName);
      if (!byName.has(key)) byName.set(key, tariff);
    }
  }

  if (rows.length) {
    for (const [key, tariff] of buildTariffIndexFromRows(rows, settings)) {
      if (!warehouseAcceptsCargoType(tariff.warehouseName, cargoType)) continue;
      if (!byName.has(key)) byName.set(key, tariff);
    }
  }
  ...
  return {
    ...
    tariffCacheStale,
    tariffFromRows: !byNameRaw.size && !boxWarehouses.length && byName.size > 0,
  };
}

/** Тарифный контекст для вкладки «Регионы» (кэш WB + строки таблицы). */
export function resolveRegionTariffContext(tariffCache, rows = [], settings = {}, meta = {}) {
  const cargoType = resolveMatrixCargoTypeFromMeta(rows, meta);
  const warehouseCargoType = resolveWarehouseSuggestionCargoType(rows, meta);
  const tariffs = resolveTariffs(tariffCache, warehouseCargoType, rows, settings);
  return {
    cargoType,
    warehouseCargoType,
    ...tariffs,
    hasTariffs: tariffs.tariffList.length > 0,
  };
}
...
/** Оценка ИЛ/ИРП после локальной отгрузки в регион с долей sharePct от заказов кабинета. */
export function projectIndicesAfterLocalStock(currentIl, currentIrp, regionSharePct) {
...
function warehouseCandidates(region, tariffList, byName, defaultTariff, resolveFo, settings, cargoType) {
  ...
  const names = new Set(geoNames.filter((name) => !isFederalDistrictLabel(name)));

  for (const name of defaultWarehousesForFo(region.foName || region.regionName || region.label)) {
    if (!isFederalDistrictLabel(name) && warehouseAcceptsCargoType(name, cargoType)) names.add(name);
  }
...
export function buildRegionSupplyRecommendations({
  ...
  const { tariffList, byName, defaultTariff, resolveFo, tariffSource, tariffCacheDate, tariffCacheStale, tariffFromRows } =
    resolveTariffs(tariffCache, warehouseCargoType, rows, settings);
