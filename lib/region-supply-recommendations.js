...
function resolveTariffs(tariffCache, cargoType = WB_CARGO.MGT) {
  ...
  return {
    ...
    cargoType,
    tariffSource: hydrated?.boxTariffs?.tariffSource || 'tariffs/box',
...
export function buildRegionSupplyRecommendations({
  ...
  const { tariffList, byName, defaultTariff, resolveFo, tariffSource, tariffCacheDate, tariffCacheStale } =
    resolveTariffs(tariffCache, cargoType);
