...
export function resolveRegionTariffContext(tariffCache, rows = [], settings = {}) {
  const cargoType = resolveMatrixCargoType(rows);
  return { cargoType, ...resolveTariffs(tariffCache, cargoType, rows, settings) };
}
...
function warehouseCandidates(region, tariffList, byName, defaultTariff, resolveFo, settings, cargoType) {
  ...
  if (!names.size) {
    for (const name of defaultWarehousesForFo(region.foName || region.label)) {
