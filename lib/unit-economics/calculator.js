...
/** Факт из отчёта; иначе глобальный % выкупа из настроек (по умолчанию 90%). */
function resolveBuyoutRate(settings, ...candidates) {
  for (const v of candidates) {
    if (v != null && v !== '' && Number.isFinite(Number(v))) {
      return Math.min(1, Math.max(0.01, num(v)));
    }
  }
  const fallback = num(settings?.buyoutRate);
  if (fallback > 0) return Math.min(1, Math.max(0.01, fallback));
  return 0.9;
}
...
export function calculateUnitEconomicsRow(input, settingsInput = {}) {
  ...
  const logisticsIndices = resolveSellerLogisticsIndices(settings);
  const priceForIrp = priceForSalesDistributionIndex({
    basePrice,
    salePrice,
    ourPrice,
  });
  const applyIndices =
    logisticsIndices.includeLogisticsIndices && !(preferActual && input.actualLogisticsRub > 0);

  const fboIndices = applyIndices
    ? applyWbLogisticsIndices(fboForwardDelivery, priceForIrp, logisticsIndices)
    : null;
  const fbsIndices = applyIndices
    ? applyWbLogisticsIndices(fbsForwardDelivery, priceForIrp, logisticsIndices)
    : null;
  const fboForwardForLogistics = fboIndices?.forwardWithIndices ?? fboForwardDelivery;
  const fbsForwardForLogistics = fbsIndices?.forwardWithIndices ?? fbsForwardDelivery;

  const subLiterTariff = volumeLiters != null && !isOverOneLiter(volumeLiters) ? subLiterTierFlatRub(volumeLiters) : null;



  const fboBuyoutRate = resolveBuyoutRate(settings, input.buyoutRateFbo, input.buyoutRate);
  const fbsBuyoutRate = resolveBuyoutRate(settings, input.buyoutRateFbs, input.buyoutRate);
  ...
  const fboLogistics = calcLogisticsCost({
    forwardDelivery: fboForwardForLogistics,
    ...
    actualLogisticsRub: input.actualLogisticsRubFbo ?? input.actualLogisticsRub,
    preferActual,
  });
  ...
  const fbsLogistics = calcLogisticsCost({
    forwardDelivery: fbsForwardForLogistics,
    returnDelivery: fbsReturnDelivery,
    buyoutRate: fbsBuyoutRate,
    returnMarkup: settings.returnLogisticsMarkup,
    useBuyoutWeighted: settings.useBuyoutWeightedLogistics,
    actualLogisticsRub: input.actualLogisticsRubFbs ?? input.actualLogisticsRub,
    preferActual,
  });

  const logisticsFbs = fbsLogistics.rub;
  const logisticsFbsSource = fbsLogistics.source;
  ...
  const fbsAvgDeliveryHoursReport =
    input.fbsAvgDeliveryHours != null && input.fbsAvgDeliveryHours !== ''
      ? num(input.fbsAvgDeliveryHours)
      : null;

  const settingsDeliveryHours =
    settings.fbsAvgDeliveryHours != null && settings.fbsAvgDeliveryHours !== ''
      ? num(settings.fbsAvgDeliveryHours)
      : null;

  const fbsAvgDeliveryHours = settingsDeliveryHours ?? fbsAvgDeliveryHoursReport;

  const actualCommissionRate = positiveRate(input.commissionActualPct);
  const useReportCommission =
    preferActual &&
    actualCommissionRate != null &&
    !(settingsDeliveryHours != null && settingsDeliveryHours > 0);
  let fbsCategoryRate;
  let fbsDeliverySurcharge;
  let fbsCategorySource;

  if (useReportCommission) {
    fbsCategoryRate = actualCommissionRate;
    fbsDeliverySurcharge = 0;
    fbsCategorySource = 'actual';
  } else {
    ({
      fbsCategoryRate,
  ...
  const fboTotalRate =
    useReportCommission
      ? actualCommissionRate + settings.extraCommissionRate
      : fboCategoryRate + settings.extraCommissionRate;
  ...
  if (salePrice > 0 && hasPurchase) {
    if (logisticsFbo != null) {
      const profitBeforeTaxFbo =
        salePrice -
        purchasePrice -
        fboCommissionRub -
        packagingCost -
      ...
      const taxFbo = computeUnitTaxes(profitBeforeTaxFbo, salePrice, settings);
      profitFbo = profitBeforeTaxFbo - taxFbo.taxRub;
      marginFbo = profitFbo / salePrice;
      profitabilityFbo = profitFbo / purchasePrice;
    }

    const profitBeforeTaxFbs =
      salePrice -
      purchasePrice -
      fbsCommissionRub -
      packagingCost -
    ...
    const taxFbs = computeUnitTaxes(profitBeforeTaxFbs, salePrice, settings);
    usnRub = taxFbs.usnRub;
    vatRub = taxFbs.vatRub;
    taxRub = taxFbs.taxRub;
    profitFbs = profitBeforeTaxFbs - taxFbs.taxRub;
    marginFbs = profitFbs / salePrice;
    profitabilityFbs = profitFbs / purchasePrice;
  }
  ...
  const logisticsCompare = compareLogisticsToActual(
    {
      logisticsFbo,
      logisticsFbs,
      baseDelivery: fboForwardForLogistics,
      fbsBaseDelivery: fbsForwardForLogistics,
      ...
      localizationIndex: logisticsIndices.localizationIndex,
      salesDistributionIndex: logisticsIndices.salesDistributionIndex,
      logisticsIrpSurcharge: fbsIndices?.irpSurcharge ?? fboIndices?.irpSurcharge ?? null,
    },
  ...
  return {
    ...
    fbsAvgDeliveryHoursReport,

    fbsDeliveryHoursSource: settingsDeliveryHours
      ? 'settings'
      : fbsAvgDeliveryHoursReport
        ? 'report'
        : null,
    ...
    fboForwardForLogistics,

    fbsForwardForLogistics,
    ...
    logisticsIndicesApplied: applyIndices,
