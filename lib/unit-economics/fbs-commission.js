...
/** Fallback: FBO + 3,5% + надбавка за время доставки (нет kgvpSupplier из API). */
...
/**
 * Комиссия FBS как в калькуляторе WB (Маркетплейс):
 * kgvpSupplier (категория) + надбавка за часы доставки на склад WB.
 * Не добавляем +3,5% к API-ставке — в кабинете WB это уже учтено в kgvpSupplier.
 */
export function resolveFbsCategoryRate({
  ...
  const fallbackBase = Number(fboCategoryRate) + Number(fbsCommissionMarkup);
  const categoryBase = hasApiRate ? Number(fbsCategoryRate) : fallbackBase;
  return {
    fbsCategoryRate: categoryBase + deliverySurcharge,
