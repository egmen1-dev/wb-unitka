...
/** НДС при УСН — всегда 5%, не отключается. */
export const FIXED_VAT_RATE = 0.05;
...
export const DEFAULT_UNIT_SETTINGS = {
  ...
  taxRate: 0.06,
  /** revenue — УСН (+ НДС) с цены продажи; wb_portal — УСН с прибыли (кальк. WB) */
  taxBaseMode: 'revenue',
  /** НДС при УСН — фикс. 5% (см. FIXED_VAT_RATE) */
  ...
  /** Среднее время доставки покупателю, ч (для надбавки к комиссии FBS; как в калькуляторе WB) */
  ...
  /** Обновлять ИЛ/ИРП из заказов WB при каждой синхронизации */
  autoSyncLogisticsIndices: true,
  /** Горизонт запаса для плана отгрузки в «Регионах», дней */
  regionStockHorizonDays: 14,
  /** Период отображения спроса по регионам (7/14/30), дней */
  regionDemandDays: 30,
};
...
function normalizeLegacySettings(settings) {
  ...
  if (s.vatRate == null || s.vatRate === 0) s.vatRate = FIXED_VAT_RATE;
  s.includeVat = true;
  s.vatRate = FIXED_VAT_RATE;
  ...
  if (!s.taxBaseMode) s.taxBaseMode = DEFAULT_UNIT_SETTINGS.taxBaseMode;
  if (!(Number(s.fbsAvgDeliveryHours) > 0)) {
    s.fbsAvgDeliveryHours = DEFAULT_UNIT_SETTINGS.fbsAvgDeliveryHours;
  }
  return s;
}

/** Миграция сохранённых настроек из localStorage / облака. */
export function migrateStoredSettings(raw) {
  if (!raw || typeof raw !== 'object') return null;
  return normalizeLegacySettings({ ...DEFAULT_UNIT_SETTINGS, ...raw });
}
...
export function settingsToForm(settings) {
  return {
    ...
    taxBaseMode: settings.taxBaseMode || 'wb_portal',
    ...
    autoSyncLogisticsIndices: settings.autoSyncLogisticsIndices !== false,
  };
}
...
export function settingsFromForm(form) {
  return mergeUnitSettings({
    ...
    taxBaseMode: form.taxBaseMode === 'revenue' ? 'revenue' : 'wb_portal',
    ...
    autoSyncLogisticsIndices: Boolean(form.autoSyncLogisticsIndices),
  });
}
