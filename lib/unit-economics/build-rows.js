...
export function buildUnitRows(
  ...
  if (settingsFromSnapshot && snapshot?.sellerAvgDeliveryHours > 0) {
    if (!(Number(settings?.fbsAvgDeliveryHours) > 0)) {
    ...
    return calculateUnitEconomicsRow(
      {
        ...
        fboStockByWarehouse: product.fboStockByWarehouse ?? [],
        ...
        commissionActualPct: product.commissionActualPct,
  ...
  return {
    ...
    localizationByNmId: snapshot?.localizationByNmId ?? null,
    ...
    matrixCargoType: snapshot?.matrixCargoType ?? 'mgt',
    tariffSource: snapshot?.tariffSource ?? 'tariffs/box',
    ...
    regionSalesPeriod: snapshot?.regionSalesPeriod ?? null,
    regionSalesError: snapshot?.regionSalesError ?? null,
    regionSalesSource: snapshot?.regionSalesSource ?? null,
    regionSalesRawRows: snapshot?.regionSalesRawRows ?? 0,
    regionSalesSynced: snapshot?.regionSalesSynced === true,
    regionSalesSnapshot: snapshot?.regionSalesSnapshot ?? null,
    regionSalesTotalQty: snapshot?.regionSalesTotalQty ?? 0,
    regionSalesSnapshotHash: snapshot?.regionSalesSnapshotHash ?? null,
    total: rows.length,
    withPurchase,
    rows,
