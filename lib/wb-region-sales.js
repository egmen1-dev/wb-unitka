...
function resolveRegionSuggestedWarehouses(entry, tariffList, cargoType) {
  ...
  if (entry.suggestedWarehouses?.length) return entry.suggestedWarehouses;
...
function mergeWarehouseDemand(primary = [], fallback = []) {
  const byKey = new Map();
  for (const entry of [...fallback, ...primary]) {
    const name = entry.warehouseName || entry.label || '';
    if (!name) continue;
    const key = normalizeText(name);
    ...
    hit.qty = Math.max(hit.qty, Number(entry.qty) || 0);
    hit.revenue = Math.max(hit.revenue, Number(entry.revenue) || 0);
    hit.sharePct = Math.max(hit.sharePct, Number(entry.sharePct) || 0);
    hit.regions = [...new Set([...hit.regions, ...(entry.regions || [])])].slice(0, 6);
    byKey.set(key, hit);
  }
  return [...byKey.values()].sort((a, b) => b.qty - a.qty || b.revenue - a.revenue);
}
...
export function enrichRegionDemandSnapshot(
  ...
  const rebuilt = rebuildWarehouseDemand(byRegion, snapshot.totalQty || 0);
  const warehouses = mergeWarehouseDemand(rebuilt, snapshot.warehouses || []);
