...
/** Средневзвешенный ИЛ по артикулам: [{ orders, localizationSharePct }]. */
export function computeLocalizationIndex(articles = []) {
  ...
  for (const item of articles) {
    ...
    if (!orders) continue;
    const share = Number(item.localizationSharePct);
    const ktr = Number.isFinite(share) ? ktrForLocalizationShare(share) : 1;
    weighted += orders * ktr;
    totalOrders += orders;
  }
...
export function computeSalesDistributionIndex(articles = []) {
  ...
  for (const item of articles) {
    ...
    if (!orders) continue;
    const share = Number(item.localizationSharePct);
    const krp = Number.isFinite(share) ? krpForLocalizationShare(share) : 0;
    weighted += orders * krp;
    totalOrders += orders;
  }
  if (!totalOrders) return null;
...
export function resolveSellerLogisticsIndices(settings = {}, snapshot = {}) {
  ...
  const localizationIndex = normalizeLocalizationIndex(
    settings.localizationIndex ?? fromSnapshot.localizationIndex,
    1
  );
  const salesDistributionIndex = normalizeSalesDistributionIndex(
    settings.salesDistributionIndex ?? fromSnapshot.salesDistributionIndex,
    0
  );
  return {
    ...
    localizationIndexSource:
      settings.localizationIndex != null
        ? 'settings'
        : fromSnapshot.localizationIndex != null
          ? 'sync'
          : 'default',
    salesDistributionIndexSource:
      settings.salesDistributionIndex != null
        ? 'settings'
        : fromSnapshot.salesDistributionIndex != null
          ? 'sync'
          : 'default',
  };
}
