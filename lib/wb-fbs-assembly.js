...
function normalizeOfficeName(value) {
  ...
  if (typeof value === 'object') {
    return String(value.name || value.officeName || value.title || '').trim();
  }
...
export function officeLabelFromOrder(order) {
  ...
  if (Array.isArray(offices) && offices.length) {
    const labels = offices.map(normalizeOfficeName).filter(Boolean);
    if (labels.length) return labels.join(', ');
  } else if (typeof offices === 'string' && offices.trim()) {
    return offices.trim();
  }
  return order?.officeId ? `офис ${order.officeId}` : '—';
}

function orderIsB2B(order) {
  return Boolean(order?.options?.isB2B || order?.isB2B);
}
...
function pickListSort(a, b) {
  return b.qty - a.qty || String(a.vendorCode).localeCompare(String(b.vendorCode), 'ru');
}

/** Фильтр готового pick-list по группе поставки (fallback, если orders нет на клиенте). */
export function filterPickListForSupplyGroup(pickList = [], groupKey = '', supplyGroups = []) {
  if (!groupKey) return pickList;
  const group = supplyGroups.find((g) => g.key === groupKey);
  if (!group?.orderIds?.length) return [];
  const orderIdSet = new Set(group.orderIds.map((id) => Number(id)));
  return pickList
    .map((row) => {
      const orderIds = (row.orderIds || []).map((id) => Number(id)).filter((id) => orderIdSet.has(id));
      if (!orderIds.length) return null;
      return { ...row, qty: orderIds.length, orderIds };
    })
    .filter(Boolean)
    .sort(pickListSort);
}

/** Агрегация pick-list по группе поставки WB (склад + габарит + crossBorder). */
export function aggregateFbsPickListForGroup(
  orders = [],
  groupKey = '',
  supplyGroups = [],
  catalogByVendor = new Map(),
  ...
  if (!groupKey) {
    return aggregateFbsPickList(orders, catalogByVendor, supplierDigitKeys);
  }
  const group = supplyGroups.find((g) => g.key === groupKey);
  if (!group?.orderIds?.length) return [];
  const orderIdSet = new Set(group.orderIds.map((id) => Number(id)));
  const filtered = orders.filter((order) => orderIdSet.has(Number(order.id)));
  return aggregateFbsPickList(filtered, catalogByVendor, supplierDigitKeys);
}
