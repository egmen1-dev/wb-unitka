...
export const PERMISSION_SECTIONS = [
  ...
  { id: 'fbs', label: 'FBS сборка', hint: 'Новые заказы и поставки FBS' },
  { id: 'feedbacks', label: 'Отзывы', hint: 'Ответы на отзывы WB с AI-черновиком' },
  { id: 'regions', label: 'Регионы', hint: 'Спрос по регионам и склады WB' },
...
/** Старый пресет до v2 — у новых сотрудников не было P&L, данных и настроек. */
const LEGACY_RESTRICTIVE_DEFAULTS = {
  calc: true,
  fbs: true,
  feedbacks: true,
  regions: true,
...
function isLegacyRestrictiveDefaults(memberDefaults) {
  return ALL_SECTION_IDS.every((id) => memberDefaults[id] === LEGACY_RESTRICTIVE_DEFAULTS[id]);
}
...
export function normalizeTeamAccess(raw) {
  ...
  // Команды, созданные до полного доступа по умолчанию — поднимаем права, если не меняли вручную.
  if (isLegacyRestrictiveDefaults(memberDefaults)) {
    for (const id of ALL_SECTION_IDS) memberDefaults[id] = true;
  }
...
export function resolveMemberPermissions(teamAccess, clientId) {
  ...
  const merged = { ...access.memberDefaults, ...overrides };
  // Участник без явных overrides и с урезанным наследием — даём полный доступ.
  if (!Object.keys(overrides).length && isLegacyRestrictiveDefaults(merged)) {
