 ...
 */
const MANAGER_STYLE_APOLOGY_PHRASES = new Set([
  'извините за брак',
  'приносим извинения',
  'сожалеем о доставленном',
]);
...
export function validateDraftQuality(text, {
  ...
  const isDefect =
    scenario?.type === 'negative_defect' ||
    scenario?.type === 'negative_quality' ||
    (managerStyle && tone === 'negative');
  ...
  for (const phrase of BANNED_PHRASES) {
    ...
    if (managerStyle && isDefect && MANAGER_STYLE_APOLOGY_PHRASES.has(phrase.toLowerCase())) continue;
