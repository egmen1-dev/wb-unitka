...
function scenarioStrategy(type, { mirrorHint }) {
  const base = {
    ...
    negative_expectation: `${mirrorHint} Ожидания могли не совпасть с назначением товара — опирайся на описание и характеристики. Предложи модель ближе к ожиданиям с артикулом.`,
...
export function validateDraftQuality(text, {
  ...
  if (answer.length < 80) issues.push('Слишком коротко (< 80 символов)');
...
const FEW_SHOT_DEFAULT = [
  ...
  'Отзыв: 1★ «Разочарован полностью»\nОтвет: «Разочарован» — жаль, что не совпало. Эта модель рассчитана на [назначение из описания]. Ближе к твоим ожиданиям — арт. [SKU].',
...
/** Шаблон без AI — с вариативностью по seed, без бан-фраз. */
export function buildTemplateDraft({ feedback, product, alternative, premiumUpsell = null, variationSeed = null }) {
  ...
  if (rating >= 4) {
    ...
    const mirror = prosSnippet ? ` Особенно «${prosSnippet}» — приятно слышать.` : '';
  ...
  if (rating >= 3) {
    ...
    const charHint = chars ? ` В карточке: ${chars.split(';').slice(0, 1).join('')} —` : '';
    const mirror = prosSnippet ? ` «${prosSnippet.split(' ').slice(0, 4).join(' ')}» отмечу.` : '';
    const alt = alternative?.article
      ...
      : '';
    return `Смешанные впечатления${firstName} —${mirror}${charHint} у этой модели своя специализация.${alt}`;
  }

  const opener = pickFromSeed(TEMPLATE_OPENERS_NEGATIVE, seed)(firstName);
  const mirror = consSnippet ? ` про «${consSnippet.split(' ').slice(0, 5).join(' ')}»` : '';
  const chars = formatCharacteristics(product?.characteristics);
  const fitHint = chars
  ...
  const altHint = alternative?.article
    ? ` Загляни на арт. ${alternative.article} — «${alternative.title}»${pricePart}, там ${alternative.analogyType === 'material' ? 'другой материал' : 'продвинутее по характеристикам'}.`
    : '';

  return `${opener}${mirror ? ` Слышу${mirror}.` : ''}${fitHint}${altHint}`;
}
