...
/** Метки контекста использования товара (промпт + UI). */
export const USAGE_CONTEXT_LABELS = {
  home: 'дом/быт',
  dacha_garden: 'дача/огород',
  kitchen: 'кухня',
  bathroom: 'ванная',
  auto: 'авто',
  office: 'офис',
...
const USAGE_CONTEXT_PATTERNS = {
  home: ['дом', 'квартир', 'быт', 'интерьер', 'комнат', 'гостин', 'спальн', 'прихож', 'балкон', 'коридор', 'уборк'],
  dacha_garden: ['дач', 'огород', 'грядк', 'полив', 'сад', 'участок', 'теплиц', 'баня', 'сауна', 'шланг', 'грабл', 'лопат'],
  kitchen: [
    'кухн',
    'готов',
    'приготов',
  ...
  bathroom: ['ванн', 'душ', 'туалет', 'сантехник', 'умывальник', 'полотенц'],
  auto: ['авто', 'машин', 'багажник', 'водител', 'гараж', 'салон машин'],
  office: ['офис', 'рабоч', 'стол', 'компьютер', 'ноутбук', 'монитор', 'принтер', 'чехол'],
  outdoor: ['улиц', 'поход', 'кемпинг', 'природ', 'на улиц', 'пикник', 'мангал', 'шашлык', 'рыбалк', 'триммер', 'бензопил'],
  travel: ['путешеств', 'дорог', 'чемодан', 'поездк', 'отпуск', 'самолёт', 'самолет', 'в поезд'],
  kids: ['ребен', 'ребён', 'дет', 'малыш', 'игруш', 'школ', 'коляск', 'подгуз'],
  sport: ['спорт', 'трениров', 'фитнес', 'йог', 'зал', 'бег', 'велосипед', 'коврик для йог'],
  edible: ['съедоб', 'вкус', 'еда', 'продукт', 'питани', 'перекус', 'конфет', 'специ', 'приправ', 'варень', 'заготовк'],
};

const PORTABLE_PATTERNS = [
  'компакт',
  'портатив',
  'переносн',
  'дорожн',
...
const USAGE_VOCABULARY = {
  home: 'дом, квартира, в быту',
  dacha_garden: 'дача, огород, на участке',
  kitchen: 'кухня, готовка, на столе',
  bathroom: 'ванная, в ванной',
  auto: 'в машине, в авто',
  office: 'офис, на работе, за столом',
  outdoor: 'на улице, на природе',
  travel: 'в поездке, в дороге',
  kids: 'для детей, с детьми',
  sport: 'на тренировке, для спорта',
  edible: 'вкус, приготовление, перекус',
};

function scoreUsageContextKeys(hay) {
  const scores = {};
  for (const [key, words] of Object.entries(USAGE_CONTEXT_PATTERNS)) {
    let score = 0;
...
/**
 * Определяет контекст использования товара по отзыву и карточке.
 * @returns {{ primary: string, primaryLabel: string, labels: string[], portable: boolean, vocabularyHint: string, summary: string }}
 */
export function detectProductUsageContext({ feedback, product } = {}) {
  const reviewHay = normalizeText(feedback?.text, feedback?.pros, feedback?.cons);
  const productHay = normalizeText(
    product?.title,
    product?.subjectName,
    product?.description,
    formatCharacteristics(product?.characteristics)
  );
  const hay = normalizeText(reviewHay, productHay);

  const reviewScores = scoreUsageContextKeys(reviewHay);
  const productScores = scoreUsageContextKeys(productHay);
  const merged = {};

  for (const key of Object.keys(USAGE_CONTEXT_PATTERNS)) {
    merged[key] = (reviewScores[key] || 0) * 2 + (productScores[key] || 0);
  }
  ...
  const primary = sorted[0]?.[0] || 'home';
  const labels = sorted.length
    ? sorted.slice(0, 3).map(([key]) => USAGE_CONTEXT_LABELS[key])
    : [USAGE_CONTEXT_LABELS.home];
  const primaryLabel = USAGE_CONTEXT_LABELS[primary] || USAGE_CONTEXT_LABELS.home;
  const portable = PORTABLE_PATTERNS.some((pattern) => hay.includes(pattern));
  const vocab = [...new Set(sorted.slice(0, 3).map(([key]) => USAGE_VOCABULARY[key]).filter(Boolean))];
  const avoidDacha =
    primary !== 'dacha_garden' && primary !== 'outdoor'
      ? ' Не пиши «на даче», если товар для другого контекста.'
      : '';
  const vocabularyHint =
    vocab.length > 0
      ? `Говори в контексте: ${vocab.join('; ')}.${avoidDacha}`
      : `Опирайся на бытовой контекст из отзыва и категории товара.${avoidDacha}`;
  const portableHint = portable ? ' · компактный/переносной' : '';
  ...
  return {
    ...
    portable,
    vocabularyHint,
    summary: `${labels.join(', ')}${portableHint}`,
  };
}
...
export const PRODUCT_TYPE_LABELS = {
  consumable: 'расходник/сменный',
  durable: 'долговечный товар',
  unknown: 'не определён',
};

/**
 * Тип товара: расходник vs долговечный — для согласованных рекомендаций.
 * @returns {{ type: 'consumable'|'durable'|'unknown', label: string, consumable: boolean, durable: boolean, upsellHint: string }}
 */
export function detectProductType(product = {}, feedback = {}) {
  ...
  let durableScore = 0;
  for (const pattern of CONSUMABLE_PATTERNS) {
    if (hay.includes(pattern)) consumableScore += 1;
  }
  for (const pattern of DURABLE_PATTERNS) {
    if (hay.includes(pattern)) durableScore += 1;
  }

  const type =
    consumableScore > durableScore ? 'consumable' : durableScore > consumableScore ? 'durable' : 'unknown';

  const upsellHint =
    type === 'consumable'
      ? 'Рекомендуй сменные блоки, расходники, аксессуары — НЕ «купи ещё такой же».'
      : type === 'durable'
        ? 'Подчеркни качество и долговечность; рекомендуй дополняющие товары, не обесценивай покупку.'
        : 'Апселл должен дополнять товар, а не противоречить его описанию.';

  return {
    type,
    label: PRODUCT_TYPE_LABELS[type],
    consumable: type === 'consumable',
    durable: type === 'durable',
    upsellHint,
  };
}

function detectRecommendationContradictions(answerHay) {
  const issues = [];
  if (/одноразов/.test(answerHay) && /(купи|загляни|новинк)/.test(answerHay)) {
    issues.push('Противоречие: товар назван одноразовым, но зовёшь купить ещё');
  }
  if (/не для постоянн/.test(answerHay) && /рекомендую ещ/.test(answerHay)) {
    issues.push('Противоречие: «не для постоянного» и приглашение купить ещё');
  }
  if (/(брак|одноразов)/.test(answerHay) && /(магазин.*полон|магазине много|много всего интересного|найдёшь и)/.test(answerHay)) {
    issues.push('Противоречие: обесцениваешь товар и зовёшь в магазин');
  }
  if (/(временн|на один раз|выброс)/.test(answerHay) && /(купи ещ|ещё такой же|такую же)/.test(answerHay)) {
    issues.push('Противоречие: товар назван временным, но предлагаешь купить такой же');
  }
  return issues;
}
...
function scenarioStrategy(type, { mirrorHint }) {
  const base = {
    ...
    negative_defect: `${mirrorHint} Эмпатия, но не «наша вина/брак». Предложи альтернативу из линейки с артикулом. Без возврата, обмена, замены и «напиши в личку».`,
...
/** Зачины для негативных отзывов — variationSeed выбирает индекс, не повторяй один зачин подряд. */
export const NEGATIVE_OPENERS = [
  'Понимаю',
  'Слышу тебя',
...
/** Частые шаблонные зачины — мягкий бан, допустимы только при variationSeed % 7 === 0. */
export const OVERUSED_NEGATIVE_OPENERS = ['ой, как жаль', 'ой, обидно'];

export function pickNegativeOpenerIndex(seed) {
  const n = Math.abs(Number(seed) || 0);
  return n % NEGATIVE_OPENERS.length;
}

export function pickNegativeOpener(seed) {
  return NEGATIVE_OPENERS[pickNegativeOpenerIndex(seed)];
}

export function allowsOverusedNegativeOpener(seed) {
  const n = Math.abs(Number(seed) || 0);
  return n % 7 === 0;
}

function startsWithOverusedNegativeOpener(answerHay) {
  return OVERUSED_NEGATIVE_OPENERS.some((p) => answerHay.startsWith(p));
}

/** Обесценивание товара или перекладывание вины на покупателя. */
const DEVALUATION_PATTERNS = [
  { re: /дефектн(?:ый|ого|ая|ое|ую|ые)\s*(?:экземпляр|товар|unit)?/i, msg: 'Не признавай «дефектный экземпляр» — объясни сценарий использования' },
  { re: /попал(?:ся|ась|ось|ись)\s+дефект/i, msg: 'Не обвиняй «попался дефект» — предложи альтернативу из каталога' },
  { re: /скорее\s+всего,?\s*попал/i, msg: 'Не своди проблему к «попался дефект»' },
  { re: /будь\s+внимательн/i, msg: 'Не перекладывай вину на покупателя («будь внимательнее»)' },
  { re: /постарайся\s+быть\s+внимательн/i, msg: 'Не перекладывай вину на покупателя' },
  { re: /в\s+следующий\s+раз/i, msg: 'Не говори «в следующий раз» — предложи подходящую модель сейчас' },
  { re: /почитай\s+отзыв/i, msg: 'Не отправляй читать отзывы перед покупкой' },
  { re: /перед\s+покупкой\s+(?:всегда\s+)?(?:стоит|лучше|надо)/i, msg: 'Не советуй «перед покупкой почитай отзывы»' },
  { re: /не\s+подош(?:ел|ла|ло|ли)\s+вам/i, msg: '«Не подошло вам» — только с переформулировкой в сценарий/задачу' },
  { re: /(?:вы|ты)\s+(?:не\s+)?(?:так\s+)?(?:выбрал|выбрала|подобрал|подобрала)/i, msg: 'Не обвиняй покупателя в неправильном выборе' },
  { re: /(?:надо|нужно|стоит)\s+быть\s+внимательн/i, msg: 'Не перекладывай вину на покупателя' },
];

function detectDevaluationIssues(hay) {
  const issues = [];
  for (const { re, msg } of DEVALUATION_PATTERNS) {
    if (re.test(hay)) issues.push(`Обесценивание: ${msg}`);
  }
  return issues;
}
...
export function validateDraftQuality(text, {
  ...
  for (const phrase of BANNED_PHRASES) {
    ...
    const isContactPhrase = /чат|напиши|подбер|личк/i.test(phrase);
    if (isContactPhrase && (allowChat || reviewAsksContact(feedback))) continue;
    issues.push(`Шаблон: «${phrase}»`);
  }

  if (/(?:^|[^а-яё])профил(?:ь|е|я|ю|и|ё|ем)(?:[^а-яё]|$)/i.test(answer)) {
    issues.push('Используй «магазин», не «профиль»');
  }

  issues.push(...detectRecommendationContradictions(hay));
  issues.push(...detectDevaluationIssues(hay));
  ...
  const isNegative = tone === 'negative' || rating <= 2;
  if (managerStyle && isNegative && startsWithOverusedNegativeOpener(hay)) {
    if (!allowsOverusedNegativeOpener(variationSeed)) {
      issues.push('Шаблонный зачин «Ой, как жаль»/«Ой, обидно» — начни с другого зачина из списка');
    }
  }
  ...
  const templateLike = issues.some((i) => i.startsWith('Шаблон') || i.startsWith('Обесценивание'));
...
4. АПСЕЛЛ — какой артикул предложить (если есть в контексте)? Он должен дополнять шаги 2–3, а не противоречить им.
...
export function buildFeedbackSystemPrompt({
  ...
  const genderLine = formatGenderPromptLine(
    buyerGender && buyerGender !== GENDER.UNKNOWN ? buyerGender : GENDER.UNKNOWN
  );
...
- НЕ называй товар одноразовым/временным и одновременно не зови купить такой же — расходникам предлагай сменные блоки и сопутку, долговечным — аксессуары.
- НЕ признавай вину продавца, НЕ извиняйся за «брак» или «ошибку».
- НЕ предлагай возврат денег, обмен, замену товара, компенсацию, «напиши в личку» для решения — только эмпатия, характеристики и альтернативный артикул.
...
«Напиши в чат заказа» или «напиши в личку» — ТОЛЬКО если покупатель сам спрашивает, как связаться.
