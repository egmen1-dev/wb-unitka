...
function accumulateArticleStats(orders, resolveWarehouseFo) {
  ...
  for (const order of orders) {
    ...
    if (!stat) {
      stat = { nmId, orders: 0, localOrders: 0, exceptionOrders: 0 };
  ...
  for (const stat of byNmId.values()) {
    ...
    localizationByNmId[stat.nmId] = {
      orders: stat.orders,
      localOrders: stat.localOrders,
      localizationSharePct: sharePct,
      isException: false,
    };
  }

  return { articles, localizationByNmId };
}
...
export function computeSellerLogisticsIndicesFromOrders(orders, { tariffByName } = {}) {
  ...
  const { articles, localizationByNmId } = accumulateArticleStats(orders, resolveWarehouseFo);
  ...
  return withWbApiToken(token, async () => {
    try {
      ...
      return {
        ...
        localizationByNmId: computed.localizationByNmId || {},
        periodDays: spanDays,
