import { parseWbAuthError } from './wb-auth-error.js';
import { normalizeWbToken } from './wb-request-token.js';
...
function getToken() {
  const token = normalizeWbToken(runtimeToken) || normalizeWbToken(process.env.WB_API_TOKEN);
  return token || null;
}
...
export async function withWbApiToken(token, fn) {
  ...
  runtimeToken = normalizeWbToken(token) || null;
  try {
    return await fn();
  } finally {
    runtimeToken = prev;
...
async function wbFetch(baseUrl, path, { method = 'GET', body = null, query = null } = {}) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    const authError = parseWbAuthError(response.status, lastText, { path });
    if (authError) {
      console.error('[wb-official-api] auth', {
        status: response.status,
        path,
        kind: authError.kind,
        detail: lastText.slice(0, 120),
      });
      const error = new Error(
        authError.kind === 'withdrawn' || authError.kind === 'scope'
          ? authError.message
          : `WB API ${response.status} ${path}: ${lastText.slice(0, 200)}`
      );
      error.code = authError.code;
      error.wbAuthKind = authError.kind;
      throw error;
    }
