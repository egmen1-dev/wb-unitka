import { filterTariffsForCargoType } from './wb-cargo-type.js';
...
/** Комиссии WB меняются разом — короткий in-memory TTL (лимит API ~1 req/min). */
const COMMISSION_MEMORY_TTL_MS = 5 * 60 * 1000;
const BOX_MEMORY_TTL_MS = 6 * 60 * 60 * 1000;
...
function writeMemoryTariff(kind, value, ttlMs = BOX_MEMORY_TTL_MS) {
  const key = tokenCacheKey();
  if (!key) return;
  memoryTariffCache.set(`${key}:${kind}`, { value, expiresAt: Date.now() + ttlMs });
}

/** Сброс in-memory кэша тарифов (после массового обновления комиссий WB). */
export function clearMemoryTariffs(kind = null) {
  if (!kind) {
    memoryTariffCache.clear();
    return;
  }
  const suffix = `:${kind}`;
  for (const key of memoryTariffCache.keys()) {
    if (key.endsWith(suffix)) memoryTariffCache.delete(key);
  }
}
...
export async function fetchAllPrices() {
  ...
  while (true) {
    ...
    for (const item of items) {
      ...
      if (nmId) pricesByNmId.set(nmId, item);
    }
...
export async function fetchPricesForNmIds(nmIds = []) {
  const pricesByNmId = new Map();
  const unique = [...new Set(nmIds.map((id) => Number(id)).filter(Boolean))];
  if (!unique.length) return pricesByNmId;
  ...
  for (let i = 0; i < unique.length; i += chunkSize) {
    ...
    if (i + chunkSize < unique.length) await sleep(650);
  }

  return pricesByNmId;
}

export function indexPricesByVendorCode(pricesByNmId) {
  const byVendor = new Map();
  for (const goods of pricesByNmId.values()) {
    const vendorCode = String(goods.vendorCode || '').trim();
    if (!vendorCode) continue;
    for (const key of vendorLookupKeys(vendorCode)) {
      if (!byVendor.has(key)) byVendor.set(key, goods);
    }
  }
  return byVendor;
}
...
export function extractPriceFromGoods(goods) {
  ...
  const sizes = goods.sizes?.length ? goods.sizes : [goods];
  let bestDiscounted = 0;
  let bestBase = 0;
  ...
  for (const size of sizes) {
    ...
    const base = Math.round(Number(size?.price || goods.price || 0));
    if (discounted > 0 && (!bestDiscounted || discounted < bestDiscounted)) {
      bestDiscounted = discounted;
      bestBase = base > discounted ? base : bestDiscounted ? bestBase : base;
    } else if (!bestDiscounted && base > 0) {
      bestBase = base;
    }
  }

  const price = bestDiscounted || bestBase;
  const oldPrice = bestBase > price && price > 0 ? bestBase : null;
...
/**
 * Комиссии WB по subjectID (доли 0..1), поля API:
 * - paidStorageKgvp → FBO/FBW (Склад WB)
 * - kgvpMarketplace → FBS (Маркетплейс) — база слайдера ~30 ч в калькуляторе прибыли
 * - kgvpSupplier → DBS/DBW (Витрина), не используем для FBS
 * Итог FBS в юнит-кальке: kgvpMarketplace + премия 30…72 ч (см. fbs-commission.js).
 * @param {{ forceRefresh?: boolean }} [options]
 */
export async function fetchCommissionTariffs({ forceRefresh = false } = {}) {
  if (!forceRefresh) {
  ...
  for (const row of data.report || []) {
    ...
    const fbw = Number(row.paidStorageKgvp || 0);
    const fbs = Number(row.kgvpMarketplace || 0);
    // Fallback на старые кеши/ответы без paidStorageKgvp
    const fboPct = fbw > 0 ? fbw : fbs;
    bySubject.set(subjectId, {
      fboCategory: fboPct / 100,
      fbsCategory: fbs / 100,
      subjectName: row.subjectName || '',
      parentName: row.parentName || '',
    });
  }
  writeMemoryTariff('commission', bySubject, COMMISSION_MEMORY_TTL_MS);
  return bySubject;
}

/**
 * Тарифы МГТ (короб) по складам WB — тот же источник, что страница
 * seller.wildberries.ru/dynamic-product-categories/delivery → «Короб».
 */
export async function fetchAllBoxTariffs(date = new Date().toISOString().slice(0, 10)) {
  ...
  const byName = buildTariffIndex(list, { cargoType: 'mgt' });
  const defaultTariff = pickDefaultBoxTariff(list, { cargoType: 'mgt' });
  const warehouses = filterTariffsForCargoType(
    list.map((row) => parseBoxTariffRow(row)).filter((t) => t.warehouseName),
    'mgt'
  );
  return {
    byName,
...
import { classifyWbCargoType } from './wb-cargo-type.js';

export function extractDimensions(card) {
  const dims = card.dimensions || {};
  const weightRaw = Number(dims.weightBrutto ?? dims.weight ?? 0);
  let weightKg = null;
  if (weightRaw > 0) {
    weightKg = weightRaw > 80 ? weightRaw / 1000 : weightRaw;
  }
  const size = {
  ...
  return {
    ...size,
    cargoType: classifyWbCargoType(size),
  };
}
