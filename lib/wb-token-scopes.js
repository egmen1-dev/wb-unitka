...
async function runFunctionalProbe(token, scopeId, apiHost) {
  ...
  if (scopeId === 'feedbacks') {
    return feedbacksApiProbe(authToken, '/ping');
  }
...
/** Категории токена WB для Юнитки (синк, тарифы, каталог). */
export const WB_UNIT_CALC_TOKEN_SCOPES = [
  {
    id: 'content',
    label: 'Контент',
    required: true,
    purpose: 'Каталог карточек, nmId, subjectId',
    apiHost: 'content-api.wildberries.ru',
    withoutScope: 'Не загрузится каталог товаров',
  },
  {
    id: 'prices',
    label: 'Цены и скидки',
    required: true,
    purpose: 'Цены продажи и скидки',
    apiHost: 'discounts-prices-api.wildberries.ru',
    withoutScope: 'Не подтянутся цены',
  },
  {
    id: 'marketplace',
    label: 'Маркетплейс',
    required: true,
    purpose: 'Склады FBS, остатки',
    apiHost: 'marketplace-api.wildberries.ru',
    withoutScope: 'Не загрузятся склады и остатки FBS',
  },
  {
    id: 'tariffs',
    label: 'Тарифы / комиссии',
    required: true,
    purpose: 'GET /api/v1/tariffs/commission и tariffs/box (common-api)',
    apiHost: 'common-api.wildberries.ru',
    withoutScope:
      'Не загрузятся комиссии и тарифы логистики — проверьте категории «Цены и скидки» / «Тарифы»',
  },
];

const UNIT_SCOPE_BY_ID = new Map(WB_UNIT_CALC_TOKEN_SCOPES.map((s) => [s.id, s]));

export function getUnitCalcTokenScope(id) {
  return UNIT_SCOPE_BY_ID.get(id) || getFeedbackTokenScope(id) || null;
}

async function runUnitCalcFunctionalProbe(token, scopeId, apiHost) {
  const authToken = normalizeWbToken(token);
  ...
  if (scopeId === 'marketplace') {
    return wbProbe(authToken, `https://${apiHost}/api/v3/warehouses`);
  }

  if (scopeId === 'tariffs') {
    return wbProbe(authToken, `https://${apiHost}/api/v1/tariffs/commission?locale=ru`);
  }

  return runFunctionalProbe(token, scopeId, apiHost);
}

/** Проверка одной категории для Юнитки. */
export async function probeWbUnitCalcTokenScope(token, scopeId) {
  const scope = getUnitCalcTokenScope(scopeId);
  if (!scope) {
    return { scopeId, ok: false, error: 'Неизвестная категория' };
  }

  const [functional, ping] = await Promise.all([
    runUnitCalcFunctionalProbe(token, scopeId, scope.apiHost),
    wbProbe(token, `https://${scope.apiHost}/ping`, { retries: 1 }),
  ]);
...
/** Проверка категорий токена для синка Юнитки (включая tariffs/commission). */
export async function probeWbUnitCalcTokenScopes(token) {
  const results = [];
  for (const scope of WB_UNIT_CALC_TOKEN_SCOPES) {
    results.push(await probeWbUnitCalcTokenScope(token, scope.id));
  }
  ...
  return {
    ...
    summary:
      missingRequired.length > 0
        ? `Не хватает категорий: ${missingRequired.map((r) => r.label).join(', ')}`
        : 'Токен подходит для синхронизации Юнитки',
  };
}
