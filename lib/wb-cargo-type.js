...
/** Тип габарита матрицы: сначала из meta синхронизации, иначе по строкам. */
export function resolveMatrixCargoTypeFromMeta(rows = [], meta = {}) {
  const fromMeta = meta?.matrixCargoType;
  if (fromMeta === WB_CARGO.MGT || fromMeta === WB_CARGO.KGT || fromMeta === WB_CARGO.SGT) {
    return fromMeta;
  }
  return resolveMatrixCargoType(rows);
}

/**
 * Тип габарита для рекомендаций складов коробов (tariffs/box).
 * Используем МГТ, кроме явной СГТ-матрицы из синхронизации WB.
 */
export function resolveWarehouseSuggestionCargoType(rows = [], meta = {}) {
  if (meta?.matrixCargoType === WB_CARGO.SGT) return WB_CARGO.SGT;
  return WB_CARGO.MGT;
}
