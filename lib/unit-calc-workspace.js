...
import { mergeWorkspacePayload } from './workspace-merge.js';
...
export async function saveWorkspace(teamCode, payload) {
  ...
  const merged = mergeWorkspacePayload(existing?.payload, payload);

  if (!existing) {
    await db`
      INSERT INTO unit_calc_workspaces (team_code, name, password_hash, payload)
      VALUES (${code}, ${'КОМАНДА'}, ${null}, ${db.json(merged)})
    `;
  ...
  const [row] = await db`
    ...
    SET payload = ${db.json(merged)}, updated_at = NOW()
    WHERE team_code = ${code}
