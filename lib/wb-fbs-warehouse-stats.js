import { isSgtWarehouseName } from './wb-cargo-type.js';
...
export function resolvePrimaryFbsShipmentContext({
  ...
  const nonSgt = activeWarehouses.filter((w) => {
    const officeName = resolveOfficeName(wbOfficesById, resolveSellerOfficeId(w));
    const hay = `${w?.name || ''} ${officeName || ''}`;
    return !isSgtWarehouseName(hay);
  });
