...
export function patchCatalogNmIdsFromReport(catalog, realization) {
  ...
  for (const [nmId, stat] of realization.byNmId) {
    const digit = realizationDigitKey(stat.vendorCode);
    if (digit.length < 3) continue;
  ...
  return catalog.map((product) => {
    const digit = realizationDigitKey(product.vendorCode);
    const hit = digit.length >= 3 ? bestByDigit.get(digit) : null;
