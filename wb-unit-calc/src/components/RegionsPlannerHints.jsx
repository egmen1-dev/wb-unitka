...
export const PLANNER_HINTS = {
  kpi: {
    ...
    periodDelta:
      'Изменение заказов по регионам относительно предыдущего снимка (localStorage) или прошлой синхронизации.',
    demandPeriod:
      'Период для отображения спроса: 7/14/30 дней. При синхронизации WB загружает выбранный период (макс. 31 дн.).',
  },
  tabs: {
    ilImpact:
    ...
    matrix:
      'Матрица ФО×SKU: спрос за период, остаток в федеральном округе, дней покрытия и статус (отгрузить / OK / не везти).',
    simulator:
      'Смоделируйте отгрузку N шт. артикула на склад WB и оцените прогноз ИЛ, ИРП и ₽/ед. с учётом локальности.',
    inTransit:
      'Товары в пути на склад WB — вычитаются из плана отгрузки. Автозагрузка через FBW Supplies API пока недоступна.',
    foWarehouses:
      'Приоритет складов по федеральным округам: рекомендация из плана поставок, доля локального покрытия спроса.',
  },
  simulator: {
    totalPerUnit: 'Логистика FBO с прогнозом ИЛ/ИРП + приёмка и хранение на горизонте запаса, на единицу.',
  },
  matrix: {
    daysOfCover: 'foStock ÷ (спрос на горизонт / горизонт дней) — сколько дней хватит остатка в ФО.',
    status: 'Отгрузить — дефицит; OK — покрывает спрос; Не везти — избыток на горизонте.',
  },
  inTransit: {
    manualKey: 'Ключ nmId::название_склада — как в плане «Отгрузить». Количество вычитается из рекомендации.',
  },
  foWarehouses: {
    coverage: 'Доля заказов округа, для которых рекомендованный склад локальный (в том же ФО).',
    delivery: 'Оценка времени доставки: локальный ФO ~70% от среднего кабинета, межрегион ~130%.',
  },
  columns: {
